...
-- Removed problematic upcoming events index with CURRENT_DATE
-- Instead, we'll rely on the main date index for filtering
...
-- View for upcoming events (next 30 days) - uses application-level filtering
...
-- Function to validate future event dates (for application use)
CREATE OR REPLACE FUNCTION is_future_event_date(check_date DATE)
RETURNS BOOLEAN AS $$
BEGIN
